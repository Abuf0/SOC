...
// clock
ckmux osc400m_scanmux (.in0(ad_osc400m), .in1(scan_clk), .sel(scan_mode), .out(clk_400m_scan));
ckmux pll100m_scanmux (.in0(ad_pll100m), .in1(scan_clk), .sel(scan_mode), .out(clk_100m_scan));
ckmux wdt32k_scanmux (.in0(wdt_32k), .in1(scan_clk), .sel(scan_mode), .out(clk_32k));
ckmux spick_scanmux (.in0(spi_ck), .in1(scan_clk), .sel(scan_mode), .out(clk_spi));
ckmux spickinv_scanmux (.in0(~spi_ck), .in1(scan_clk), .sel(scan_mode), .out(clk_spi_inv)); // todo

div_clk #(.DIV_WID(5)) clk_div_0 (.clk(clk_400m_scan), .rstn(rstn_400m), .div_dat_even(5'd20), .clk_div_o(clk_20m));
div_clk #(.DIV_WID(3)) clk_div_1 (.clk(clk_20m), .rstn(rstn_400m), .div_dat_even(3'd4), .clk_div_o(clk_5m));

ckgate_cell ckgt_reg_inst(.clkin(clk_5m), .enable(reg_ckgt_case), .scan_en(scan_enable), .clkout(clk_reg));
ckgate_cell ckgt_tim_inst(.clkin(clk_20m), .enable(timer_enable), .scan_en(scan_enable), .clkout(clk_tim));

ckgate_cell ckgt_isp_inst(.clkin(clk_400m_scan), .enable(isp_enable), .scan_en(scan_enable), .clkout(clk_isp));
ckgate_cell ckgt_fifo_inst(.clkin(clk_400m_scan), .enable(fifo_enable), .scan_en(scan_enable), .clkout(clk_fifo));

ckgate_cell ckgt_afe_inst(.clkin(clk_100m_scan), .enable(afe_enable), .scan_en(scan_enable), .clkout(clk_afe));

sync_level afe_enable_sync(.clk(clk_100m_scan), .rstn(rstn_100m), .level_in(~da_stb_en), .level_out(afe_enable));
...
sync_reset rstn_100m_inst(.clk(clk_100m_scan), .async_reset(async_reset_alon), .sync_rstn(rstn_100m));
