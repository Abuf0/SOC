...
## 设计
- #### clock gate设计
  - 可以分为静态时钟门控(即用户通过寄存器使能)和动态时钟门控(即内部状态控制门控使能)；
  - 动态时钟门控设计需谨慎，看情况留后门；
  - 时钟门控结构：
    
    `genpart_ckgt ckgt_inst(.clk(clk), .enable(enable), .scan_enable(scan_enable), .gclk(clk_gt));`

    > - 其中enable如果是非静态信号，需要同步到clk时钟域；
    > - scan_enable保证了scan测试下clk_gt不会因为enable翻转而gate掉，即此时enable'=enable  | scan_enable;
    > - 时钟门控waveform，下降沿采样enable  ![alt text](image-7.png)
    > - 动态时钟门控的设计需要注意`ckgt_en_case_list 宁可多杀，不可放过`
    >   - 假设使用“A拉高”行为作为ckgt_en，如果需要采到“A拉低”那一拍的行为，则需要enwiden门控窗口，即ckgt_en = A_sync | A_sync_d1; 相对的，如果只关注“A拉高”的行为，则无需enwiden门控窗口，即ckgt_en = A_sync；
    >   - 如果有跨时钟同步行为，例如CLKA domain有STATE_A结束时，送出end_a --> 异步时钟域CLKB domain，握手后返还end_b --> CLKA domain去clear或者update一些逻辑。如果此时ckgt_en_a = STATE_A，那么gated clk_gt，无法同步&采样到握手信号end_b，导致功能错误，因此需要enwiden门控窗口；
    > - gate越forward，Low Power效果越好；
- #### test mode的启动
...
- #### 同步器要用common lib的sync，便于验证和cdc check能通过脚本或者工具抓取sync
  - 选用同步器时，需要考虑时钟域和信号特性：
    - 电平信号同步（即较宽信号同步3-edge）可以用打两拍；
    - 脉冲电平同步，需要考虑脉冲间隔，间隔小的需要用握手同步方式；
...
- #### SPI需要考虑自身频率和寄存器系统时钟的同步速率匹配
- #### SPI相关时刻要注意SCK的非常开性
- #### I2C和SPI通讯模式的切换
  - ChelseaA中，由于PAD数量够，每次复位后，根据GPIO tie 0/1，选择通讯方式为SPI/I2C；（此时SCK复用为SCL,MOSI复用为SDA）
  - CardiffC中，由于PAD数较少，每次复位后，根据CSN的值为H(SPI)/L(I2C)来判断通讯模式；（问题：ESG把芯片打回复位时，主控不知道，可能还在以SPI读写数据，此时CSN为L，芯片以为被置为I2C，此时主控可能还在MOSI输出信号，如果复用MOSI作为SDA，CardiffC可能也会在SDA输出信号，造成短路；因此复用MISO为SDA，此时至少主控为输入，最多造成短时间的漏电（CSN总会被拉高））
...
- #### 溢出判断、饱和处理
  ...
  ```
  // 假设两个Nbit有符号数相加，判断溢出后做饱和处理 //
  logic signed [N-1:0] add_a;
  logic signed [N-1:0] add_b;
  logic signed [N-1:0] sum_sat;
  // 方法1
  logic signed [N-1:0] sum;
  logic upov_flag;
  logic downov_flag;
  assign sum = add_a + add_b;
  assign upov_flag = add_a[N-1] && add_b[N-1] && ~sum[N-1];
  assign downov_flag = ~add_a[N-1] && ~add_b[N-1] && sum[N-1];
  assign sum_sat = upov_flag?   {1'b0,{(N-1)1'b1}} :
                   downov_flag? {1'b1,{(N-1)1'b0}} : sum;

  // 方法2
  logic signed [N:0] sum_ex;
  assign sum_ex = add_a + add_b;  // 注意此时是：sum_ex[N:0] = {add_a[N-1],add_a} + {add_b[N-1],add_b};
  assign ov_flag = (sum_ex[N] != sum_ex[N-1]);  //如果没有符号位溢出，真实符号位==截位符号位；且无需考虑零溢出问题//
  assign sub_c_sat = ov_flag?  {sum_ex[N],{(N-1){~sum_ex[N]}}} : sum_ex[N-1:0];
  //sum_ex[N]才是真实符号位
  ```
...
- #### 绝对值
  - 有符号数计算时都是用补码计算，正数的补码=原码，负数的补码={符号位，数据位按位取反}

- #### 数据移位
  - 举例：
    ```
    assign data_shift = data >>> (rg_scale[2:0] + 1'b1);
    // 当rg_scale=3'b111时，想要的效果是data右移8bit，但编译器会认为 >>> 后边是3bit的值，因此只取了(rg+1)的低3bit，即8->0，引发问题。必须改为：
    assign data_shift = data >>> ({1'b0,rg_scale[2:0]}+1'b1);
    // 或者重新声明一个完整位宽的scale变量，data >>> scale
    ```
...
## 中后端问题
- #### 后仿真毛刺
  主要针对DA信号需要考虑；数字电路内部constrainted合理的话，是无需考虑的；
  - case 1：MUX逻辑生成毛刺
  
    `Y = ~sel & A + sel & B`
    
    - 毛刺原因：【竞争冒险】~sel 是 sel经过INV后产生，即使A=B=0，sel从0跳1时，Y会产生一个高电平glitch；
    - 解决方案：RTL阶段手动例化mux的stdcell；
  - case 2：复位R2Q延时差导致毛刺

    `Y = pos & neg, 在pos为高、neg为低时复位，pos复位值为0，neg复位值为1`
    - 毛刺原因：neg复位比pos复位快，复位后夹出一个glitch；
    - 解决方案：很难从RTL处理，和下级模块确认、后端时序约束（eg. set data check）

- #### set_data_check
  - 写在SDC中，不影响综合结果，但会给到PR工具，PR工具会根据该指令优化布局布线；
  - `set_data_check -from [related pin] -to [constrained pin] -setup/hold xx`
  - 其中related pin对应clk path，constrained pin对应data path；

- #### DFT覆盖率
  - 一般只需要保证时钟/复位能受scan控制，就能保证DFFs能受控制。DFT工具能根据DFF的I/O端推断出组合逻辑各个门电路端口的值。
  - 当组合逻辑很长很深时，内部有些点无法被控制（UC）或观测（U0），需要DFT在无法观测的点前后引出wire插入scan来增加控制，或者RTL阶段，插寄存器打断组合逻辑做优化。

    
### PMU
以ChleseaB为例，控制系统工作模式的切换，根据状态机控制系统上下电；
系统有Normal、Sleep、DeepSleep三种工作模式；有五个电压域；
检测OCP电路状态，在pswitch过流时切断供电开关，并产生中端唤醒并告知CPU；

GM388：
EPC根据COREPACTIVE，向CPU请求切换状态，并和core完成握手，保证安全上下电；
控制ISO、HAMMER、TRIKLE、RET等行为的先后顺序；
控制多组时钟的门控；
设计多组状态机分别管理电源状态切换、EPU与CPU握手、上下电关键信号切换流程、时钟门控开关等过程；
