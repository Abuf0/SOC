...
assign wn_addr_co3 = wn_addr_co2 + (wn_addr_sta << 1);  
...
always_ff@(posedge clk or negedge rstn) begin
    ...
    else if(state_radix4 && (state_c != LAST_STAGE) && tcnt == 8) begin    
        if(radix_loop_cnt == radix_num-1) begin
            if(group_loop_cnt == group_num-1)
                wn_addr_sta <= 'd0;
            else 
                wn_addr_sta <= wn_addr_sta + twid;  
        end
    end
    else if(state_pre) begin
        if(tcnt_loop_end)
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        wn_addr <= 'd0; 
    else if(state_radix4 && (state_c != LAST_STAGE)) begin
        if(tcnt == 0)
            wn_addr <= wn_addr_co2;
        else if(tcnt == 1)
            wn_addr <= wn_addr_si2;
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        wn_rd <= 1'b0;
    else if(state_radix4 && (tcnt <= 5) && ~(state_c == LAST_STAGE))
        wn_rd <= 1'b1;
    else if(state_pre && (tcnt==0 || tcnt==1))    
        wn_rd <= 1'b1;
...
assign wn_rd_cnt = wn_rd_d1?   tcnt[3:0] : 4'd8;
//-------------------todo--------------------------//
always@(*) begin
    stage_num = 2;
    case(fft_len)
