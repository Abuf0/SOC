module moce_cfft #(
    ...
    input        [ADDR_WIDTH-1:0]           rg_src_data_base    ,
    input        [ADDR_WIDTH-1:0]           rg_dest_data_base   ,
...
logic [15:0] fft_len;
logic [ADDR_WIDTH-1:0]       src_data_base ;
logic [ADDR_WIDTH-1:0]       dest_data_base ;
...
assign need_move = (rg_src_data_base != rg_dest_data_base);

typedef enum logic [2:0] {IDLE, PRE, RADIX4, POST, REVERSE, MOVE} fft_state_t;
    ...
    case(fft_state_c)
        ...
        RADIX4: begin
            if(need_4by2) begin
                ...
                if(radix4_done)
                    fft_state_n = rg_bitreverse_flag?   REVERSE : (need_move?   MOVE : IDLE );
                else
                    fft_state_n = RADIX4;
            end
        end
        POST:   fft_state_n = post_done?    (rg_bitreverse_flag?    REVERSE : (need_move?   MOVE : IDLE )) : POST;
        REVERSE: fft_state_n = reverse_done?    (need_move?   MOVE : IDLE ) : REVERSE;
        MOVE: fft_state_n = move_done?  IDLE : MOVE;
...
assign move_start_pre = (fft_state_c != MOVE) && (fft_state_n == MOVE);

assign cfft_done_pre = (fft_state_c != IDLE) && (fft_state_n == IDLE);

assign fft_len = (fft_state_c == MOVE)?  rg_fft_len : (need_4by2?  (rg_fft_len>>1) : rg_fft_len);
...
assign src_data_base = radix4_cnt?  ((rg_src_data_base >> 2) + rg_fft_len) : (rg_src_data_base >> 2);
assign dest_data_base = (rg_dest_data_base >> 2);
...
radix4 # (
    ...
    .src_data_base      (src_data_base       ) ,
    .dest_data_base     (dest_data_base      ) ,
    .wn_base            (wn_base             ) ,
    .rev_base           (rev_base            ) ,
