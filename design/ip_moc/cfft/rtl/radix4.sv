...
logic move_stage_done;
logic move_stage_done_pre;
...
logic [ADDR_WIDTH-1:0] move_src_addr;
logic [ADDR_WIDTH-1:0] move_dest_addr;
...
always @(*) begin
    ...
    case(state_c) 
        ...
        MOVE_STAGE :  state_n = move_stage_done?        IDLE : MOVE_STAGE;
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        tcnt <= 'd0;
    else if((state_c != IDLE) && (state_c != MOVE_STAGE))
...
always_ff@(posedge clk or negedge rstn) begin   
    if(~rstn)
        move_stage_done <= 1'b0;
    else if(state_c == MOVE_STAGE && move_stage_done_pre)
        move_stage_done <= 1'b1;
    else 
        move_stage_done <= 1'b0;
end
...
always_ff@(posedge clk or negedge rstn) begin   
    if(~rstn)
        move_done <= 1'b0;
    else if(state_c == MOVE_STAGE && state_n == IDLE)
        move_done <= 1'b1;
    else 
        move_done <= 1'b0;
end
...
`ifdef DEBUG
    ...
    if(((tcnt>=0 && tcnt<=7) || (tcnt==tcnt_num-1)) && ~last_stage_last_radix) 
        add3_a = radix_loop_end?   (src_data_base >> 1) : re_addr_a;
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn) 
        ...
        move_src_addr <= src_data_base;
    else if(state_move && data_rd)
        move_src_addr <= move_src_addr + 1;
end
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn) 
        ...
        move_dest_addr <= dest_data_base;
    else if(state_move && data_wr)
        move_dest_addr <= move_dest_addr + 1;
end
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn) 
        ...
        data_addr <= src_data_base + rev_addr_next;
    else if(state_move)
        data_addr <= (rw_flag)? move_dest_addr : move_src_addr;
end
...
assign data_wdata = (state_move)?   data_rdata : buff4;
...
assign wn_addr_co1 = wn_base + (wn_addr_sta << 1);
assign wn_addr_si1 = wn_addr_co1 + 1'b1;
assign wn_addr_co2 = wn_addr_co1 + (wn_addr_sta << 1) ;
assign wn_addr_si2 = wn_addr_co2 + 1'b1;
assign wn_addr_co3 = wn_addr_co2 + (wn_addr_sta << 1);  // todo : can reuse ADD2
assign wn_addr_si3 = wn_addr_co3 + 1'b1;
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        wn_addr_sta <= 'd0; // fix
    else if(pre_start)
        wn_addr_sta <= (wn_base >> 1);
    else if(fft_start)
        wn_addr_sta <= 'd0;
...
assign wn_wr = 1'b0;
assign wn_wmask = {DATA_WIDTH{1'b1}};
//-------------------todo--------------------------//
always@(*) begin
    stage_num = 2;
    case(fft_len)
