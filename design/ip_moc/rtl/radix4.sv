module radix4#(
    ...
    output logic [DATA_WIDTH-1:0]       lb_wdata            ,
    output logic                        lb_wr               ,
    output logic                        lb_rd               ,
    output logic [ADDR_WIDTH-1:0]       lb_addr
);
...
// FSM
typedef enum logic  [1:0] {IDLE, FIRST_STAGE, MIDDLE_STAGE, LAST_STAGE} state_t;
...
always @(*) begin
    state_n = state_c;
    case(state_c) 
        IDLE:         state_n = fft_start?              FIRST_STAGE : IDLE;
...
assign radix_loop_end = tcnt_loop_end && (radix_loop_cnt == radix_num-1);
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        radix_loop_cnt <= 'd0;
...
always_ff@(posedge clk or negedge rstn) begin   // false fft_done for dbg
    if(~rstn)
        fft_done <= 1'b0;
...
always_ff@(posedge clk or negedge rstn) begin
    ...
    else if(tcnt_loop_end && state_c != IDLE)
...
always_ff@(posedge clk or negedge rstn) begin   // todo except last radix
    if(~rstn)
        data_rd <= 1'b0;
    else if(~(|tcnt[4:3]) && (state_c != IDLE) && ~last_stage_last_radix)
        data_rd <= 1'b1;
    else 
        data_rd <= 1'b0;
end
...
always_ff@(posedge clk or negedge rstn) begin   // todo except first radix
    if(~rstn)
        data_wr <= 1'b0;
    else if((tcnt >= 19 && tcnt<=26) && (state_c != IDLE) && ~first_stage_first_radix)
        data_wr <= 1'b1;
    else 
        data_wr <= 1'b0;
end
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        wn_addr_sta <= WN_BASE;
    else if((state_c != IDLE) && (state_c != LAST_STAGE) && tcnt == 14) begin    // condition: >= 13 && < tcnt_num-3
        if(first_radix)
            wn_addr_sta <= WN_BASE;
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        wn_addr <= 'd0;
    else if(((state_c!=IDLE) && (state_c != LAST_STAGE)) || last_stage_first_radix) begin
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        wn_rd <= 1'b0;
    else if((state_c != IDLE) && ((tcnt >= tcnt_num-3) || (tcnt<=13)) && ~(state_c == LAST_STAGE && ~last_stage_first_radix))
        wn_rd <= 1'b1;
    else 
        wn_rd <= 1'b0;
end
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        mem1_wr <= 1'b0;
    else if((tcnt>=3 && tcnt<=10) && (state_c != IDLE)) 
        mem1_wr <= 1'b1;
    else
        mem1_wr <= 1'b0;
end
...
always_ff@(posedge clk or negedge rstn) begin
    if(~rstn)
        mem1_rd <= 1'b0;
    else if((tcnt>=11 && tcnt<=18) && (state_c != IDLE)) 
        mem1_rd <= 1'b1;
    else
        mem1_rd <= 1'b0;
end
